...
// Mengambil semua task dalam database
json_t *todo_find_all (void)
{
    sqlite3_mutex_enter(sqlite3_db_mutex(db));
    sqlite3_stmt *stmt;
    json_t *todos, *todo, *id, *text, *status, *due;
    int rc;
...
json_t *todo_find_by_id (int todo_id)
{
    ...
    int rc;
    sqlite3_stmt *stmt;
    json_t *todo = NULL, *id, *text, *status, *due;
